...
const Shop = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('http://localhost:8080/products'); // Adjust API endpoint accordingly
        ...
        {products.length > 0 ? (
          products.map((product) => (
            <div key={product.id} className="border rounded-lg p-3 shadow-md hover:shadow-lg">
              ...
              <button
                onClick={() => handleAddToCart(product.id)} // Navigate to Product Detail page with product ID
